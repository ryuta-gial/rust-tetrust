// これは、blockというモジュールから必要な型や関数をインポートしているという意味
// です。以下のように解釈できます。
// block_kind モジュールから WALL という型を W という名前でインポート
// gen_block_7 関数をインポート
// BlockColor 型をインポート
// BlockKind 型をインポート
// BlockShape 型をインポート
// BLOCKS 定数をインポート
// COLOR_TABLE 定数をインポート
...
// フィールドサイズ
// 外部からアクセス可能な可視性にするには、pubキーワードを使う
...
// 構造体
// init()メソッドは、新しいPositionインスタンスを作成し、xには5を、yには0を設定しています。
// init()メソッドは、Positionを返す
impl Position {
...
impl Game {
    pub fn new() -> Game {
        ...
        let mut game = Game {
            ...
            // 新しいPositionを作成する
            // xには5が、yには0が設定された新しいPositionインスタンスが作成されます。
            pos: Position::init(),
...
// ゴーストの座標を返す
// テトリスのゴーストと呼ばれるブロックを描画するための座標を計算しています。
// fieldは参照で渡され、
// 変更されることはないので、&を付けて借用（borrowing）している

fn ghost_pos(field: &Field, pos: &Position, block: &BlockShape) -> Position {
    // ghost_pos 変数に現在のブロックの位置 pos をコピーします
    let mut ghost_pos = *pos;
    //while ループで、現在のブロックを ghost_pos の下に一段落とした座標 new_pos を作成し、
    // その座標が衝突していない場合、つまり現在のブロックが field 上に落下できる場合は
    //  ghost_pos を new_pos に更新します。この処理を繰り返し、最終的に ghost_pos がゴーストの座標となります。
    while {
...
// 与えられた↑Game構造体から、フィールド、現在のブロック、ホールド中のブロック、次に出現するブロック、そしてスコアを取得
// .. 構文は、分解していないフィールドを無視するため
// テトリスのフィールドを描画するために使用される
// & 借用
pub fn draw(
    Game {
        field,
    ...
    // 描画用フィールドの生成
    // *fieldをコピーしてfield_bufにバインディングする
    // Game構造体のfieldフィールドのコピーを生成しています。
    //field_bufは、元のfieldとは別のメモリ空間に作成された配列です。
    //mutキーワードは、field_bufが可変であることを示しています。
...
pub fn is_collision(field: &Field, pos: &Position, block: &BlockShape) -> bool {
    for y in 0..4 {
        for x in 0..4 {
            // フィールド上の座標とブロックの形状を比較し、
            // ブロックとフィールドの両方に同時に存在する場合は衝突していると判定しています。
            if y + pos.y >= FIELD_HEIGHT || x + pos.x >= FIELD_WIDTH {
                continue;
            }
            // ブロックがフィールドの範囲内にあり、
            // かつその場所に既に別のブロックが存在する場合に、衝突していると判断しています。
            // block_kindはブロックの種類を表す列挙型
